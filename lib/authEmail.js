...
export async function CustomsendVerificationRequest(params) {
  const { identifier, url, provider, site } = params;
  const transport = createTransport(provider.server);
  const result = await transport.sendMail({
    to: identifier,
    from: provider.from,
    subject: `Magic-link for DuckPass.`,
    text: text(url, site),
    html: html({ url, identifier, site }),
  });
...
function html(params) {
  const { url, identifier, site } = params;
...
function text(url, site) {
  return `Sign in to ${site} with DuckPass\n${url}\n\n By DuckLabs.`;
}
