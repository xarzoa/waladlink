...
export async function generateMetadata({ params }) {
  ...
  const res = await fetch(`${process.env.BASE_URL}/${user}/get`).then((res) => res.json())
  return {
    title: `${res.data?.name}'s WaladLink`,
    icons: {
      icon: {
        url: res.data?.avatar || 'https://images.ducklabs.xyz/optimize/waladlinkweb/assets/aa48804b-78f9-47e6-9768-a1a9a26dbed0.png?bucket=ducklabs&width=100&height=100'
      }
    }
  }
}
...
export default async function UserPage({ params }) {
  ...
  if(user.theme === "clouds"){
    return(
      <CloudsTheme user={user}/>
    )
  }
  if(user.theme === "graphite"){
    return(
      <GraphiteTheme user={user}/>
    )
  }
  return(
